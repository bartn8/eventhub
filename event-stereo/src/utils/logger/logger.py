...
class ExpLogger:
    ...
    def colormap_image(self,x,vmax=None,cmap='Spectral_r'):
    ...
    def add_image(self, tag, image, global_step):
        ...
        if (image.dim() == 3 and image.shape[0] == 1):
            image = self.colormap_image(image,cmap='Spectral_r')
        elif (image.dim() == 4 and image.shape[1] == 1):
            image = self.colormap_image(image.squeeze(1),cmap='Spectral_r')
        elif (image.dim() == 4 and image.shape[1] == 3):
            image = image[0]
        else:
            raise ValueError("Image tensor must be of shape (1, H, W) or (3, H, W) or (1, 1, H, W) or (1, 3, H, W)")
        
        self._tensor_log.add_image(tag=tag, img_tensor=image, global_step=global_step)
