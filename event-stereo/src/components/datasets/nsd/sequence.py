...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, baseline, split, sampling_ratio, event_cfg, disparity_cfg, 
        ...
        self.width = width if width is not None else NSD_WIDTH
        self.height = height if height is not None else NSD_HEIGHT
        self.seq_size = args.seq_size if args is not None and hasattr(args, 'seq_size') else 0

        # self.sequence_name = "_".join(root.split('/')[-3:])  # Use last three parts of the path as sequence name
        #self.sequence_name = f"{self.sequence_name}_b{self.baseline}"
        self.sequence_name = "_".join(root.split('/')[-3:-1]) # Use third and second last parts of the path as sequence name
        ...
        # Timestamps
        if split in ['train_h', 'validation_h', 'train_v', 'train', 'validation', 'curation', 'validation_v', 'train_z', 'validation_z',
                     'train_hvz', 'validation_hvz']:
        ...
        # Transforms
        if split in ['train_h', 'train_v', 'train_z', 'train', 
                     'train_hvz']:
        ...
        elif split in ['validation_h', 'validation_v', 'validation_z', 'validation_hvz', 'validation', 'curation']:
            self.transforms = transforms.Compose([
