...
class DisparityDataset(torch.utils.data.Dataset):
    _PATH_DICT = {
        ...
        'scale_n_shifts': 'scale_n_shifts.json',
        'depth': 'rendered_frames_depth_median',
        'depth_mesh': 'rendered_frames_depth_mesh',
    ...
    def __init__(self, root, seq_size, baseline, width, height, use_mesh_depth=False, load_images=False, curation_epe_threshold=-1, **kwargs):
        self.root = root
        self.seq_size = seq_size
        self.baseline = baseline
        self.width = width 
        self.height = height
        self.use_mesh_depth = use_mesh_depth
        ...
        # # Convert scale and shifts
        # _scale_n_shifts = json.load(open(os.path.join(root, self._PATH_DICT['scale_n_shifts']), 'r'))[str(self.baseline)]
        # self.scales = {timestamp: _scale_n_shifts["scale"][i] for i, timestamp in enumerate(self.timestamps_dict['C'])}
        # self.shifts = {timestamp: _scale_n_shifts["shift"][i] for i, timestamp in enumerate(self.timestamps_dict['C'])}
        ...
        for i,j in zip(['ao', 'alpha', 'sizeconf', 'depth', 'depth_mesh'], ['ao_C', 'alpha_C', 'sizeconf_C', 'depth_C', 'depth_mesh_C']):
        ...
        assert 'depth_mesh_C' in self.timestamp_to_path, "Mesh depth paths not found. Check the dataset structure."
    ...
    def __getitem__(self, timestamp):
        ...
        if self.use_mesh_depth:
            _disp = self.load_tensor(self.timestamp_to_path['depth_mesh_C'][timestamp], scale_factor=1000.0)  # Depth is stored in 16-bit PNG, so scale by 1000 to get meters
        else:
            _disp = self.load_tensor(self.timestamp_to_path['depth_C'][timestamp], scale_factor=1000.0)  # Depth is stored in 16-bit PNG, so scale by 1000 to get meters
        ...
        for key in self._SCALE_DICT:
            ...
            if self.load_images is False and 'rgb' in key:
                continue
        ...
        if self.use_mesh_depth:
            output_dict['myconfidence'] = np.where(_disp > 0, 1.0, 0.0)
        else:
