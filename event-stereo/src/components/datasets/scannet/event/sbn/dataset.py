...
# from ....events_utils.utils import render

class EventDataset(torch.utils.data.Dataset):
    _PATH_DICT = {
        #'timestamp': 'timestamps.txt',
        'left': 'rendered_events',
        'right': 'rendered_events'
    }
    _LOCATION = ['left', 'right']
    NO_VALUE = None

    def __init__(self, root, baseline, stack_method, **kwargs):
        self.root = root
        self.baseline = baseline
        self.stack_method = stack_method
        self.event_slicer = {}

        # Determine view directions and file suffixes based on checking if .h5 files exist
        #Default (Center = Left and CenterRight = Right)
        views = [('left', ''), ('right', f"_CR{abs(self.baseline):.2f}")]

        all_exist = True
        for location, shift_str in views:
            event_path = os.path.join(root, f"{self._PATH_DICT[location]}{shift_str}")
            all_exist = all_exist and os.path.exists(event_path)
            if all_exist:
                self.event_slicer[location] = EventSlicer(event_path, **kwargs)
        
        if not all_exist:
            raise FileNotFoundError(f"Event files not found in {root}. Expected files: {[os.path.join(root, f'{self._PATH_DICT[location]}{shift_str}.h5') for location, shift_str in views]}")
        ...
        for location in self._LOCATION:
            self.stack_function[location] = getattr(stack, stack_method)(dropout_rnd_state=self.dropout_rnd_state, noise_rnd_state=self.noise_rnd_state, **kwargs)
    ...
    def _pre_load_event_data(self, timestamp):
        ...
        for location in self._LOCATION:
            event_data[location] = self.stack_function[location].pre_stack(event_data[location], timestamp)
