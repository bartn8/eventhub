...
class EventSlicer(torch.utils.data.Dataset):
    def __init__(self, event_root, rectify_map_root, num_of_event):
        ...
        self.h5_fs = None
        try:
            self.h5_fs = h5py.File(event_root, 'r')
        except OSError as e:
            raise OSError(f"{str(e)}; {event_root}") from e

        self.ms_to_idx = np.asarray(self.h5_fs['ms_to_idx'], dtype='int64')
        self.t_offset = int(self.h5_fs['t_offset'][()])
        self.t_final = int(self.h5_fs['events/t'][-1]) + self.t_offset
        self.min_time = int(self.h5_fs['events/t'][num_of_event]) + self.t_offset
        self.max_time = int(self.h5_fs['events/t'][-1]) + self.t_offset
        self.total_event = len(self.h5_fs['events/t'])

        try:
            with h5py.File(rectify_map_root, 'r') as h5_rect:
                self.rectify_map = h5_rect['rectify_map'][()]
        except OSError as e:
            raise OSError(f"{str(e)}; {rectify_map_root}") from e

    # Ensure to close file when the object is destroyed
    def __del__(self):
    ...
    def get_events_base_number(self, number_of_event: int,  t_end_us: int) -> Dict[str, np.ndarray]:
        ...
        # # Handle None values from ms2idx using searchsorted as fallback
        # if t_start_ms_idx is None:
        #     t_start_ms_idx = np.searchsorted(self.h5_fs['events/t'], t_start_us, side='left')
        # if t_end_ms_idx is None:
        #     t_end_ms_idx = np.searchsorted(self.h5_fs['events/t'], t_end_us, side='right')
        ...
        h5f = self.h5_fs
        events['t'] = (
            np.asarray(h5f['events/{}'.format('t')][t_start_us_idx:t_end_us_idx], dtype=np.uint64)
            + np.uint64(self.t_offset)
        )
