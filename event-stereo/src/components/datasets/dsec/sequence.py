...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, split, sampling_ratio, event_cfg, disparity_cfg, 
        ...
        # Timestamps
        if split in ['train', 'validation', 'trainval', 'render', 'proxy', 'val_outdoor_day', 'val_outdoor_night', 'train_outdoor_day', 'train_outdoor_night']:
        ...
        # Transforms
        if split in ['train', 'trainval', 'proxy']:
        ...
        elif split in ['train_outdoor_day', 'train_outdoor_night']:
            self.transforms = transforms.Compose([
                transforms.Resize(event_module=event_module,
                                  disparity_module=disparity_module,
                                  img_height=1080//2, img_width=1440//2),
                transforms.RandomCrop(event_module=event_module,
                                      disparity_module=disparity_module,
                                      crop_height=crop_height, crop_width=crop_width),
                transforms.ToTensor(event_module=event_module,
        ...
        elif split in ['validation', 'test', 'render', 'val_outdoor_day', 'val_outdoor_night']:
            self.transforms = transforms.Compose([
    ...
    with open(csv_file) as csvfile:
        ...
        for row in data_reader:
            if row[0].isnumeric():
                timestamp = int(row[0])
                index = int(row[1])
                assert timestamp not in timestamp_to_index, f"Duplicate timestamp {timestamp}"
