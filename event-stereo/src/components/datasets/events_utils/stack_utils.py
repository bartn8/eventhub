...
def events2ToreFeature(x, y, ts, pol, sampleTimes, k, frameSize):
    ...
    addEventIdx = np.logical_and(ts >= priorSampleTime, ts <= currentSampleTime)#< or <=???
    ...
    # _newPosTore = np.full(frameSize + (k,), np.inf)#HxWxK

    # for t, i, j in zip(pts_diff, py, px):
    #     newPosTore[i - 1, j - 1] = np.partition(np.concatenate(([t], newPosTore[i - 1, j - 1, : k - 1])), k - 1)[:k]

    _fast_queue(newPosTore, px, py, pts_diff)

    # np.testing.assert_almost_equal(_newPosTore[np.logical_not(np.isinf(_newPosTore))], newPosTore[np.logical_not(np.isinf(newPosTore))])

    p = np.logical_and(addEventIdx, pol <= 0)
    px, py, pts_diff = x[p], y[p], currentSampleTime - ts[p]

    newNegTore = np.full(frameSize + (k,), np.inf)#HxWxK
    # _newNegTore = np.full(frameSize + (k,), np.inf)#HxWxK

    # for t, i, j in zip(pts_diff, py, px):
    #     newNegTore[i - 1, j - 1] = np.partition(np.concatenate(([t], newNegTore[i - 1, j - 1, : k - 1])), k - 1)[:k]

    _fast_queue(newNegTore, px, py, pts_diff)
    
    # np.testing.assert_almost_equal(_newNegTore[np.logical_not(np.isinf(_newNegTore))], newNegTore[np.logical_not(np.isinf(newNegTore))])
...
@numba.jit(nopython=True)
def fast_to_eros(x, y, t, p, height, width, k_o, decay_factor):
    """
    Fast EROS orientation map update using numba JIT compilation.
    
    Parameters:
        x, y: event coordinates arrays
        t: timestamps array (not used in EROS but kept for interface compatibility)
        p: polarity array (not used in EROS but kept for interface compatibility)
        height, width: sensor dimensions
        k_o: kernel radius
        decay_factor: precomputed decay factor (0.3^(1/k_o))
    
    Returns:
        Updated orientation map
    """
    ...
    for i in range(len(x)):
        ...
        x_start = max(0, vx - k_o)
        x_end = min(width, vx + k_o + 1)
        y_start = max(0, vy - k_o)
        y_end = min(height, vy + k_o + 1)
        
        # Update orientation values in the neighborhood
        for nx in range(x_start, x_end):
            for ny in range(y_start, y_end):
                if nx == vx and ny == vy:
                    # Set event pixel to 1.0
                    orientation_map[ny, nx] = 1.0
                else:
                    # Apply exponential decay
                    orientation_map[ny, nx] *= decay_factor
    
    return orientation_map


@numba.jit(nopython=True)
def fast_to_teros(x, y, t, p, height, width, k_o, decay_factor):
    """
    Fast Tencode-EROS orientation map update using numba JIT compilation.
    
    Parameters:
        x, y: event coordinates arrays
        t: timestamps array (assume ordered timestamps between 0 and 1)
        p: polarity array 
        height, width: sensor dimensions
        k_o: kernel radius
        decay_factor: precomputed decay factor (0.3^(1/k_o))
    ...
    orientation_map = np.ones((3, height, width), dtype=np.float32)
        ...
        for nx in range(x_start, x_end):
            for ny in range(y_start, y_end):
                if nx == vx and ny == vy:
                    # Set event pixel to 1.0
                    if polarity > 0:
                        orientation_map[0, ny, nx] = 1.0
                        orientation_map[2, ny, nx] = 0.0
                        orientation_map[1, ny, nx] = np.exp(timestamp-1)
                    else:
                        orientation_map[0, ny, nx] = 0.0
                        orientation_map[2, ny, nx] = 1.0
                        orientation_map[1, ny, nx] = np.exp(timestamp-1)
                else:
                    # Apply exponential decay to time
                    orientation_map[0, ny, nx] = (orientation_map[0, ny, nx] * decay_factor)
                    orientation_map[1, ny, nx] = (orientation_map[1, ny, nx] * decay_factor)
                    orientation_map[2, ny, nx] = (orientation_map[2, ny, nx] * decay_factor)
