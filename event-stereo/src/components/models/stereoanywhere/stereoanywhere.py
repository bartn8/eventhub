...
class StereoAnywhere(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        self.args.n_downsample = 2 # Fix n_downsample to 2 (quarter res) #if not hasattr(args, "n_downsample") else args.n_downsample
        ...
        self.args.features_output_dim = 256 if not hasattr(args, "features_output_dim") else args.features_output_dim
        ...
        self.args.things_to_freeze = [] if not hasattr(args, "things_to_freeze") else args.things_to_freeze

        #Override self.args using kwargs if provided
        ...
        self.fnet = BasicEncoder(input_dim=self.args.in_channels, output_dim=self.args.features_output_dim, norm_fn='instance', downsample=self.args.n_downsample)

        # self.lightfnet = FeatureV4(img_channels=1, n_downsample=self.args.n_downsample)
    ...
    def freeze_for_finetuning(self):
        ...
        for meta_layer in self.args.things_to_freeze:
            if meta_layer in _freeze_dict:
                for layer in _freeze_dict[meta_layer]:
                    for param in layer.parameters():
                        param.requires_grad = False
                
    def forward(self, bins2, bins3, mde2, mde3, feats2 = None, feats3 = None, iters=12, test_mode=False):
        B, C, H, W = bins2.shape
        W_lowres = W // (2 ** self.args.n_downsample)

        bins2, bins3 = bins2.contiguous(), bins3.contiguous()
        ...
        inp_list = [list(conv(i).split(split_size=conv.out_channels//3, dim=1)) for i,conv in zip(inp_list, self.context_zqr_convs)] # type: ignore

        fmap2, fmap3 = self.fnet([bins2, bins3])
        
        if feats2 is not None and feats3 is not None:
            fmap2 = torch.cat([fmap2, feats2], dim=1)
            fmap3 = torch.cat([fmap3, feats3], dim=1)

        #fmde2, fmde3 = self.lightfnet(mde2_vollowres), self.lightfnet(mde3_vollowres)
        ...
        global_scale_left, global_shift_left = normalized_depth_scale_and_shift(torch.cat([mde2_lowres, mde3_lowres], 1), torch.cat([coarse_dispmono2_lowres, coarse_dispmono3_lowres], 1), torch.cat([coarse_dispmonoconf2_lowres, coarse_dispmonoconf3_lowres], 1))
